...
スタッキューでは、スタックとキューを用いてお望みの情報管理構造をつくります。たとえば普段のタスクリストとして queue1、割り込み用に stack1 の 2 本を使うことを考えます。こうすると「普段は queue1 で先入れ先出し的に処理するが、割り込みが来たら stack1 で後入れ先出し的に処理しきる」運用が可能となります。単純ですが、普段のタスクと割り込みの両方に対応できます。

### 原則
スタッキューではいくつか原則があります。前提と言い換えても良いです。これらはすべて理解しなければなりません。

- 情報管理とは
    - 個人の、個人による、個人のための情報を管理すること
    - 自己完結させるものであり、他者に見せる・見られることは想定しない
- 情報とは
    - TODO、タスク、アイデア、後で読むものなどあらゆるものが対象ですが、**テキストとして** 表現されています。また何らかの行動を必ず伴わせます
- 管理とは
    - 情報の追加、閲覧、検索、処理（行動に移す）、別のスタッキューへの移動を含みます
    - **編集操作は含みません**
    - **処理と移動は同時には行いません**。情報を取り出したときに行なえることは、1:処理して消す、2:処理せずに移動する、のいずれかです
    - なお移動する場合に限り、編集も可能です
...
- (特に想定する情報)
    - このパターンで扱う「情報」について、特に想定するものがあれば書きます
- (情報の処理)
    - どのスタッキューを処理するかを書きます
...
- (情報の追加)
    ...
    - 外部からの追加と言えます。たいていは高レイヤー側に置いたスタッキューで受け付けます
- (情報の移動)
...
上記はスタック stack1 とキュー queue1、queue2 の計 3 本から成っています。
...
上記も同様に 3 本から成りますが、`|` で区切った各々はグループと呼びます。といっても厳密な意味はなくて、似た役割を持つスタッキューを整理するものです。この場合、stack1 と queueXX はそれぞれ別の役割を持たせている可能性が高いと読めます。

```
stack1 | queue1 | queue2
```

上記もやはり同じ 3 本から成りますが、今度は queue1 と queue2 のグループが分かれており、両者に別の役割を持たせているだろうと読めます。
...
FIFO ドリブンは複数のキューを使い分けていましたが、レイターパターンではスタックを置いています。つまり「あとでやる」ものは、後に入れたものから取り出されます。溜まると底が見えなくなるので、早めに処理しようという気になりやすいです。
...
- (情報の追加)
    - queue1 に追加する
- (情報の移動)
    - queue1 からグループ 2 に移す。その際、いつまで先送りしていいかを判断して適切なスタッキューに移す
...
グループ 2 のスタッキューは 7 本以内にしてください。これ以上多いと判断が煩雑になって形骸化します。また、できるだけ少ない方が良いです。「必要最小限の本数で済むためには？」を追求するのがポイントです。
...
いつ、どのキューを処理するかは定めていません。厳格にやってもいいですし、「今日は polling2 だけ見よう」のように雑に運用しても構いません。このパターンはとにかく「3 回だけ頑張る」ことを保障しています。
...
- (特に想定する情報)
    - 作業的なタスク。作業的とは「標準的な人材と状況なら指定時間内に完了できる」または「時間で区切られており指定時間の間過ごし続ける」こと。製造業と接客業に多い
- (情報の処理)
...
また私が整理した「タスクの属性一覧」も参考になるかもしれません:
